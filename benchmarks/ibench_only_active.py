...
print(
    time_summary.sort_values(["suite", "bench", "inputsize", "median_ms"]).to_string(
        index=False
    )
)

alloc_df = df[
    (df["criterion"].str.lower() == "allocated") & (df["unit"].str.lower() == "bytes")
].copy()
...
alloc_summary = (
    alloc_per_invocation.groupby(group_final, dropna=False)[
        "median_bytes_per_invocation"
    ]
    .median()
    .rename("median_bytes")
    .reset_index()
...
print("\n***** Allocation summary  *****")
print(
    alloc_summary.sort_values(
        ["suite", "bench", "inputsize", "median_bytes"]
...
os.makedirs("output_id5553-only-active", exist_ok=True)
...
for col, label in [
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("output_id5553-only-active", f"{col}.png"))
    plt.close()
